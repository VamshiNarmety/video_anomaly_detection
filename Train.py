...
if args.method == 'pred':
    ...
    model = convAE(args.c, args.t_length, args.msize, args.fdim, args.mdim, attn_decoder=args.attn_decoder)
else:
...
for epoch in range(args.epochs):
    ...
    start = time.time()
    pbar = tqdm(train_batch, desc=f"Epoch {epoch+1}/{args.epochs}", leave=False)
