...
class convAE(torch.nn.Module):
    def __init__(self, n_channel =3,  t_length = 5, memory_size = 10, feature_dim = 512, key_dim = 512, temp_update = 0.1, temp_gather=0.1, attn_decoder='none'):
        ...
        # If decoder gating is requested, create attention gates for each skip level
        if self.attn_decoder == 'gate':
            # skip3 channels = 256, skip2 = 128, skip1 = 64
            self.att_gate3 = AttentionGate(F_g=feature_dim, F_l=256)
            self.att_gate2 = AttentionGate(F_g=feature_dim, F_l=128)
            self.att_gate1 = AttentionGate(F_g=feature_dim, F_l=64)
    ...
    def forward(self, x, keys,train=True):
        ...
        if train:
            ...
            # If decoder gating is enabled, gate skip features using memory read
            if self.attn_decoder == 'gate':
                # split updated_fea into (query_feat, mem_read)
                d = self.feature_dim
                query_feat = updated_fea[:, :d, :, :]
                mem_read = updated_fea[:, d:, :, :]

                gated_skip3, a3 = self.att_gate3(skip3, mem_read)
                gated_skip2, a2 = self.att_gate2(skip2, mem_read)
                gated_skip1, a1 = self.att_gate1(skip1, mem_read)

                # aggregate attention maps (downsampled to mem size) and apply to mem_read
                # a1/a2/a3 are alpha maps downsampled to mem_read spatial size
                alpha_mem = (a1 + a2 + a3) / 3.0
                mem_read_mod = mem_read * alpha_mem

                updated_fea_mod = torch.cat((query_feat, mem_read_mod), dim=1)
                output = self.decoder(updated_fea_mod, gated_skip1, gated_skip2, gated_skip3)
            else:
